...
def turnoff(update, context):
  context.bot.send_message(chat_id=update.effective_chat.id, text="Lights turned off")
...
def turnon(update, context):
  context.bot.send_message(chat_id=update.effective_chat.id, text="Lights turned on")
...
def input_message(update, context):
  ...
  if text == 'turn on':
    send_value(1)
    context.bot.send_message(chat_id=update.effective_chat.id,text="Lighta turned on")
    context.bot.send_photo(chat_id=update.effective_chat.id,photo='https://img.icons8.com/plasticine/2x/light-on.png')
  elif text == 'turn off':
    send_value(0)
    context.bot.send_message(chat_id=update.effective_chat.id,text="Lights turned off")
...
def Hi(update,context):
  ...
  turnoff :To turn off lights
  turnon  :To turn on  lights
'''
